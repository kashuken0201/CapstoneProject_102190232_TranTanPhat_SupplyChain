...
cloneSamplesRepo() {
    ...
    if [ -d test-network-supply-chain ]; then
        # if we are in the fabric-samples repo, checkout corresponding version
        echo "==> Already in fabric-samples repo"
    elif [ -d CapstoneProject_102190232_TranTanPhat_SupplyChain ]; then
...
download() {
    ...
    if [ -d CapstoneProject_102190232_TranTanPhat_SupplyChain ]; then
       DEST_DIR="CapstoneProject_102190232_TranTanPhat_SupplyChain"
    fi
