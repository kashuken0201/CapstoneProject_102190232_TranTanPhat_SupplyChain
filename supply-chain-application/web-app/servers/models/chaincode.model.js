...
const getChainInfo = async (isManufacturer, isConsumer, information) => {
  ...
  const contractRes = await network.queryChaincode(
    networkObj,
    "GetChainInfo",
    info
  );
  ...
  while (true) {
    try {
      ...
      const res = {
        ...
        Timestamp:
          contractRes.data.data[0].payload.header.channel_header.timestamp,
        Creator:
          contractRes.data.data[0].payload.header.signature_header.creator
            .mspid,
...
const getBlockByTxID = async (isManufacturer, isConsumer, information) => {
  ...
  const contractRes = await network.queryChaincodee(
    networkObj,
    "GetBlockByTxID",
    channel,
    info
