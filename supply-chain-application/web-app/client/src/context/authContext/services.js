...
import { logOut, signInFailure, signInStart, signInSuccess } from "./authAction";
  ...
  try {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        organization: userlogin.data.user.organization,
        role: userlogin.data.user.role,
      })
    );
    dispatch(signInSuccess({ user: userlogin.data.user }));
  } catch (err) {
